...
#if os(Windows)
public typealias foundation_char_t = WCHAR
#else
public typealias foundation_char_t = Int8
#endif
...
open class FileManager : NSObject {
    ...
    */
    ...
    internal func __fileSystemRepresentation(withPath path: String) throws -> UnsafePointer<foundation_char_t> {
        let len = CFStringGetMaximumSizeOfFileSystemRepresentation(path._cfObject)
        if len != kCFNotFound {
            let buf = UnsafeMutablePointer<foundation_char_t>.allocate(capacity: len)
    ...
    internal func _fileSystemRepresentation<ResultType>(withPath path: String, _ body: (UnsafePointer<foundation_char_t>) throws -> ResultType) throws -> ResultType {
        let fsRep = try __fileSystemRepresentation(withPath: path)
        defer { fsRep.deallocate() }
        return try body(fsRep)
    }

    internal func _fileSystemRepresentation<ResultType>(withPath path1: String, andPath path2: String, _ body: (UnsafePointer<foundation_char_t>, UnsafePointer<foundation_char_t>) throws -> ResultType) throws -> ResultType {
    ...
    open func string(withFileSystemRepresentation str: UnsafePointer<Int8>, length len: Int) -> String {
        ...
        return NSString(bytes: str, length: len, encoding: encoding)!._swiftObject
    }

    open func _string(withFileSystemRepresentation str: UnsafePointer<foundation_char_t>, length len: Int) -> String {
#if os(Windows)
      let encoding = String.Encoding.utf16LittleEndian.rawValue
#else
      let encoding = String.Encoding.utf8.rawValue
#endif
      return NSString(bytes: str, length: len * MemoryLayout<foundation_char_t>.size, encoding: encoding)!._swiftObject
    }
